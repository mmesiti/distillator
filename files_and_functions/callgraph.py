...
def get_graph_from_callgraph_info(callgraph_info):
    ...
    call_graph = [(name, callees)
                  for name, filename, storage, callees in callgraph_info]
    return dict(call_graph)


def get_file_function_dict_from_callgraph_info(callgraph_info):
    '''Assuming that names are unique: can be false! '''
    file_function_dict = dict(
        (name, filename) for name, filename, callees in callgraph_info)
...
def get_callgraph(
    ...
    source_files = include.source_files(source_filename_filter_function,
                                        sombrero_root)
    print("Getting AST for all files...")
...
class CallGraph:
    def __init__(self, call_graph_info, root_info):
        ...
        self.defined_function_names = set(
            name for name, filename, storage, callees in call_graph_info)
    ...
    def find_func_candidates_by_name(self, func_name):
        return [(name, filename, storage, callees)
                for name, filename, storage, callees in self.call_graph_info
                if name == func_name]

    def find_children(self, parent_name, filename):
        '''
        this method finds the functions
        that are called by
        the function with name"parent_name"
        which is defined in file "filename".
        ...
        # precisely identify the parent
        # given its name, and the file where it is defined.
        parent_candidates_info = self.find_func_candidates_by_name(parent_name)
        if not parent_candidates_info:
            return None
        try:
            parent_info = next(info for info in parent_candidates_info
                               if info[1] == filename)
        ...
        callee_infos = [
            callee_info for callee in callees
            for callee_info in self.find_func_candidates_by_name(callee)
        ]

        name_file_storage_children = [
            (name_callee, filename_callee, storage_callee)
            for name_callee, filename_callee, storage_callee, _ in callee_infos
            if storage_callee != ['static'] or filename_callee == filename
        ]

        name_children = [n for n,f,s in name_file_storage_children]
        repeated_names = [ n for n in name_children
                           if name_children.count(n) > 1]

        # Assuming that a repetition can happen
        # only if one of the
        # If storage is static, it takes precedence.
        name_file_children = [ (n,f) for n,f,s in name_file_storage_children
                               if n not in repeated_names
                               or (s == ['static']  and f == filename)]
    ...
    def _iterate_children(self,
                          parent_name,
                          filename,
                          stack=[]):  # TODO: fix to use full callgraph info
        '''
        parent_name: name of the function
        filename: filename where the function is defined
        '''
        if parent_name in self.defined_function_names:
            ...
            for child_name, child_filename in children_info:
                if (child_name, child_filename) not in stack:
                    yield from self._iterate_children(
                        child_name, child_filename,
                        stack + [(child_name, child_filename)])
                else:
                    print(
                        f"Recursion, {child_filename}:{child_name} in stack:")
                    print("==============")
                    for stack_child_filename, stack_child_name in stack:
                        print(f"{stack_child_filename}:{stack_child_name}")
                    print(f"{child_filename}:{child_name}")
                    print("==============")
        else:
            self.undefined_functions.add(parent_name)

    def __iter__(self):
        yield from self._iterate_children(self.root_function, self.root_file,
                                          [(self.root_function,self.root_file)])


def get_all_defined_functions(callgraph_info):
    return set(f"{filename}:{name}"
               for name, filename, storage, callees in callgraph_info)
